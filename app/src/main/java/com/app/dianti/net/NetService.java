...
public final class NetService {
    ...
    private NetService(@NonNull Context appContext) {
        mAppContext = appContext;

        mHttpClient = new OkHttpClient.Builder().build();
        final Retrofit retrofit = new Retrofit.Builder()
                .client(mHttpClient)
                .baseUrl(AppContext.SERVER_URL)
    ...
    public void getElevatorList(
        ...
        if (!getHttp()){
         call = mService.getElevatorList(token, code, ele_name, use_dep_name, maintain_dep_name, page);
        }else{
            call = mService2.getElevatorList(token, code, ele_name, use_dep_name, maintain_dep_name, page);
        }
...
//            AppContext.SERVER_URL_2 = localServerAddress2;
    ...
    public void getElevatorList(
        ...
        final Call<KResponse<ElevatorList>> call;
            if (!getHttp()){
                 call = mService.getElevatorList(token, page, code, ele_name, use_dep_name, maintain_dep_name);

            }else {
                call = mService2.getElevatorList(token, page, code, ele_name, use_dep_name, maintain_dep_name);
            }
        handleCall(call, listener);
    }
...
//                .build();
//
//        final Call<String> call = mService.uploadImage(body);
//        call.enqueue(new Callback<String>() {
//            @Override
//            public void onResponse(Call<String> call, Response<String> response) {
//                if (response.isSuccessful()) {
